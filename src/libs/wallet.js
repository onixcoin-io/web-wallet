...
switch (config.getNetwork()) {
    case 'testnet':
        network = onix.networks.onix_testnet
        break
    case 'mainnet':
        network = onix.networks.onix
        break
}
...
export default class Wallet {
    ...
    async setInfo() {
        ...
        this.info.orc20 = info.orc20Balances.map(token => {
    ...
    static generateCreateTokenTx(
        ...
        return onix.utils.buildCreateContractTransaction(
            wallet.keyPair,
            orc20TokenCode + encodedParam.substr(2),
            gasLimit,
            gasPrice,
            fee,
            utxoList
    ...
    static async generateSendToContractTx(
        ...
        return onix.utils.buildSendToContractTransaction(
            wallet.keyPair,
            contractAddress,
            encodedData,
            gasLimit,
    ...
    static async generateTx(wallet, to, amount, fee, utxoList) {
        ...
        return onix.utils.buildPubKeyHashTransaction(
            wallet.keyPair,
            to,
            amount,
            fee,
    ...
    static restoreFromMnemonic(mnemonic, password) {
        ...
        const hdNode = onix.HDNode.fromSeedHex(seedHex, network)
    ...
    static restoreFromWif(wif) {
        return new Wallet(onix.ECPair.fromWIF(wif, network))
    }

    static async restoreHdNodeFromLedgerPath(ledger, path) {
        const res = await ledger.onix.getWalletPublicKey(path)
        const compressed = ledger.onix.compressPublicKey(
            Buffer.from(res['publicKey'], 'hex')
        )
        const keyPair = new onix.ECPair.fromPublicKeyBuffer(compressed, network)
        const hdNode = new onix.HDNode(
            keyPair,
