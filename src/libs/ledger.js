import onixJsLib from 'onixjs-lib'
import btcApp from '@ledgerhq/hw-app-btc'
import transportU2f from '@ledgerhq/hw-transport-u2f'
import BigNumber from 'bignumber.js'
import SafeBuffer from 'safe-buffer'
import OPS from 'onix-opcodes'
...
export default class Ledger {
  ...
  static async connect() {
    const onix = new btcApp(await transportU2f.create())
    // ensure in Onix App
    const pubkeyRes = await onix.getWalletPublicKey(Ledger.defaultPath)
    if (pubkeyRes.bitcoinAddress[0] !== 'X') {
      throw 'Not Onix App'
    }
    return new Ledger(onix)
  }
  ...
  static async generateTx(keyPair, ledger, path, from, to, amount, fee, utxoList, rawTxFetchFunc = () => {}) {
    ...
    const pubkeyRes = await ledger.onix.getWalletPublicKey(path)
    ...
    const inputs = []
    const paths = []
    const selectUtxo = onixJsLib.utils.selectTxs(utxoList, amount, fee)
    const rawTxCache = {}
    for(let i = 0; i < selectUtxo.length; i++) {
      ...
      inputs.push([
        await ledger.onix.splitTransaction(rawTxCache[item.hash]),
        item.pos
      ])
    }
    const outputs = new onixJsLib.TransactionBuilder(keyPair.network)
    ...
    return await ledger.onix.createPaymentTransactionNew(inputs, paths, undefined, outputsScript)
  }
  ...
  static async generateSendToContractTx(keyPair, ledger, path, from, contractAddress, encodedData, gasLimit, gasPrice, fee, utxoList, rawTxFetchFunc = () => {}) {
    ...
    const outputs = new onixJsLib.TransactionBuilder(keyPair.network)
    const contract =  onixJsLib.script.compile([
      OPS.OP_4,
