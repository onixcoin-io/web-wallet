<template>
  <div>
    <p align="center">{{ $t('mnemonic.input_words') }}</p>
    <v-card-text>
      <v-container grid-list-md>
        <v-layout wrap>
          <v-flex xs4 v-for="i in length" :key="i">
            <v-text-field autocapitalize="none" color="white"
    ...
    <v-card-actions>
      <v-spacer></v-spacer>
      <v-btn class="grey--text text--darken-3" color="amber" dark @click="inputMnemonicWords" :disabled="notFinishInput">{{ $t('common.confirm') }}</v-btn>
    </v-card-actions>
    </v-layout>
  </div>
</template>
