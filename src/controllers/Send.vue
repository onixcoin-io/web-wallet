<template>
  <v-card class="grey darken-4">
    ...
    <v-card-text>
      <v-form>
        ...
        <v-layout>
          ...
          <v-flex xs3>
            <v-select color="amber" :items="tokens" v-model="symbol" bottom>
        ...
        <v-text-field
          color="white" v-if="symbol !== 'ONIX'"
          label="Gas Price (1e-8 ONIX/gas)"
          v-model="gasPrice"
        ></v-text-field>
        <v-text-field
          color="white" v-if="symbol !== 'ONIX'"
          ...
          v-model="gasLimit"
        ></v-text-field>
        <v-text-field color="white" label="Fee" v-model.trim="fee" required></v-text-field>
      </v-form>
    </v-card-text>
    <v-card-actions class="pa-4">
      <v-spacer></v-spacer>
      <v-btn class="grey--text text--darken-3" color="amber" dark @click="send" :disabled="notValid">{{
        $t("common.confirm")
      }}</v-btn>
    </v-card-actions>
    <v-dialog v-model="confirmAddressDialog" persistent width="unset">
      <v-card class="grey darken-3">
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn class="amber--text darken-1" flat @click="confirmAddress">{{
            $t("common.confirm")
          }}</v-btn>
          <v-btn
    ...
    <v-dialog v-model="confirmSendDialog" persistent width="unset">
      <v-card class="grey darken-3">
        <v-card-title class="justify-center">
          <span class="headline">
    ...
    <v-dialog v-model="addTokenDialog" persistent width="unset">
      <v-card class="grey darken-3">
        <v-card-title class="justify-center">
          <span class="headline">
            Token
        ...
        <v-card-text>
          <v-container grid-list-md>
            <v-layout wrap>
              <v-flex xs12>
                <v-text-field
                  color="white" :label="$t('send.token_address')"
  ...
  methods: {
    ...
    async confirmAddress() {
      ...
      try {
        ...
        } else if (orc20.checkSymbol(this.symbol)) {
          if (wallet.extend.ledger) {
            this.rawTx = 'Please confirm tx on your ledger...'
          }
          const token = orc20.getTokenBySymbol(this.symbol)
          const encodedData = orc20.encodeSendData(
            token,
            this.address,
            this.amount
          )
    ...
    async confirmSend() {
      ...
      try {
        ...
        } else if (regex.test(res.message)) {
          this.$root.error(`Send Failed: Increase the transaction fee and try again. <p style=" display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; width: 300px;"> ${res.message}` , true, 0)
        } else if (regex1.test(res.message)) {
          this.$root.error(`Send Failed: Decrease the transaction fee and try again. <p style=" display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; width: 300px;"> ${res.message}` , true, 0)
        } else {
          this.$root.error(`Send Failed : <p style=" display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; width: 300px;"> ${res.message}`, true, 0)
        }
    ...
    async searchAddToken() {
      ...
      try {
        const tokenInfo = await orc20.fetchTokenInfo(this.addTokenAddress)
        ...
        if (
          ...
          e.message === 'this contract is not a orc20 token'
        ) {
    ...
    initTokens() {
      const tokenList = [{ text: 'ONIX', value: 'ONIX' }]
      orc20.getTokenList().forEach(token => {
